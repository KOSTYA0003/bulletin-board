<x-app-layout title="Вход">
    <div class="max-w-md mx-auto">
        ...
        <form method="POST" action="{{ route('login.submit') }}" class="bg-white rounded-xl shadow-md border border-gray-100 p-6 space-y-5">
            @csrf
            ...
            <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-lg transition cursor-pointer">Войти</button>
        </form>

        <p class="text-center text-gray-600 mt-6">
            Нет аккаунта?
            <a href="{{ route('register') }}" class="text-indigo-600 hover:text-indigo-800 hover:underline">Зарегистрироваться</a>
        </p>
    </div>
</x-app-layout>